...
namespace Snowlight.Communication
{
    public class ClientMessage
    {
        ...
        public byte[] ReadBytes(bool IsBreakString = false)
        {
            if (IsBreakString)
            {
                string[] strArray2 = Constants.DefaultEncoding.GetString(this.mBody).Split(new char[] { '\x00b3', '\x00b2' }, StringSplitOptions.RemoveEmptyEntries)[this.int_0].Split(new char[] { '\x00b3' }, StringSplitOptions.RemoveEmptyEntries);
                return Constants.DefaultEncoding.GetBytes(strArray2[this.int_1++]);
            }
            string[] strArray3 = Constants.DefaultEncoding.GetString(this.mBody).Split(new char[] { '\x00b3', '\x00b2' }, StringSplitOptions.RemoveEmptyEntries);
