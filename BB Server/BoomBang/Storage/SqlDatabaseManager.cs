...
namespace Snowlight.Storage
{
    public static class SqlDatabaseManager
    {
        ...
        public static void ProcessMonitorThread(object state)
        {
            if (ClientCount > mMinPoolSize)
            {
                lock (mSyncRoot)
                {
                    ...
                    {
                        //Output.WriteLine("(Sql) Disconnected " + ToDisconnect.Count + " inactive client(s).", OutputLevel.DebugInformation);
                    }
                }
            }
        }
        ...
        public static void SetClientAmount(int ClientAmount, string LogReason = "Unknown")
        {
            ...
            //Output.WriteLine("(Sql) Client availability: " + ClientAmount + "; modifier: " + Diff + "; reason: " + LogReason + ".", OutputLevel.DebugInformation);
        }
