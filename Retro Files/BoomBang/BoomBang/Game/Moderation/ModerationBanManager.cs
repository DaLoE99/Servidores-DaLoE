namespace BoomBang.Game.Moderation
{
    ...
    public static class ModerationBanManager
    {
        ...
        public static void BanUser2(SqlDatabaseClient MySqlClient, uint UserId, uint BanType, string MessageText, string Moderator, double Length, string IP)
        {
            MySqlClient.SetParameter("userid", UserId);
            MySqlClient.SetParameter("bantype", BanType);
            MySqlClient.SetParameter("reason", MessageText);
            ...
            MySqlClient.SetParameter("timestampex", UnixTimestamp.GetCurrent() + Length);
            MySqlClient.SetParameter("moderator", Moderator);
            MySqlClient.SetParameter("ip", IP);
            MySqlClient.ExecuteNonQuery("INSERT INTO baneos (id_usuario,tipo_baneo,detalles,timestamp,timestampex,moderador,direccion_ip) VALUES (@userid,@bantype,@reason,@timestamp,@timestampex,@moderator,@ip)");
            lock (object_0)
            {
                list_0.Add(UserId);
                dictionary_0.Add(UserId, new BanDetails(UserId, BanType, MessageText, UnixTimestamp.GetCurrent(), UnixTimestamp.GetCurrent() + Length, Moderator, IP));
            }
        }
        ...
        public static void Unban(SqlDatabaseClient MySqlClient, uint ID)
        {
            MySqlClient.SetParameter("ID2", ID);
            MySqlClient.ExecuteNonQuery("DELETE FROM baneos WHERE id_usuario = @ID2");
            ReloadCache(MySqlClient);
        }
       ...
       public static void BanHammer2(SqlDatabaseClient MySqlClient, uint UserID, string reason, string actor)
        ...
        public static void BanHammer3(SqlDatabaseClient MySqlClient, uint UserID, string reason, string actor, string IP)
        {
            BanUser2(MySqlClient, UserID, 3, reason, actor, 99999, IP);
            ReloadCache(MySqlClient);
        }
